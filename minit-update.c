  ...
  if (read_outfd(buf, BUFLEN) == 1) {
    ...
    if (buf[0] == '0')
      buffer_puts(buffer_2,
                  "expected '1' from minit, got '0' - minit too old?\n");
  }
  ...
  for (j=0; j<=maxprocess; ++j) { /* skip duplicates */
   if(!strcmp(root[j].name,service)) return 0;
    ...
    case sizeof(tmp):
      ...
      addprocess(&tmp);
      if (verbose)
        buffer_puts(buffer_1, " saved.\n");
      break;
    case 1:
      if (verbose)
        buffer_puts(buffer_1,
                    " failed - minit has no information on this service\n");
#if 0  
    break;
   default:
...
int main(int argc, char **argv) {
  ...
  if (infd < 0 || outfd < 0)
    die("could not open " MINITROOT "/in or " MINITROOT "/out\n");
  ...
  if (verbose)
    buffer_putsflush(buffer_1,
                     "minit-update: telling minit to execve itself\n");
...
ssize_t read_outfd(void *buf, size_t count) {
  ...
  if (br < 0)
    buffer_puts_strerror("Error reading from outfd: ");
  return br;
}
