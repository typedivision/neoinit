...
void free(void *x) {
  if ((char *)x >= malloc_buf && (char *)x < malloc_buf + sizeof(malloc_buf))
    return;
  __libc_free(x);
}
...
int addprocess(struct process *p) {
  if (maxprocess + 1 >= processalloc) {
    struct process *fump;
    processalloc += 8;
    if ((fump = (struct process *)realloc(
             root, processalloc * sizeof(struct process))) == 0)
      return -1;
    root = fump;
  }
...
void sulogin() { /* exiting on an initialization failure is not a good idea for
                    init */
  char *argv[] = {"sulogin", 0};
...
int main(int argc, char *argv[]) {
...
#ifdef UPDATE
  {
    ...
    fl.l_len = 0;
    fl.l_pid = 0;
    if ((0 == fcntl(infd, F_GETLK, &fl)) && (fl.l_type != F_UNLCK))
      doupdate = 1;
  }
...
/*	write(1,buf,str_len(buf)); write(1,"\n",1); */
        ...
        if (((buf[0] != 'U') && buf[0] != 's') &&
            ((idx = findservice(buf + 1)) < 0) && strcmp(buf, "d-"))
#else
        ...
        if (buf[0] == 'h') {
          ...
          write(outfd, "1:", 2);
          {
            ...
            for (i = 0; i < HISTORY; ++i)
              if (history[i] != -1)
                write(outfd, root[history[i]].name,
                      str_len(root[history[i]].name) + 1);
            write(outfd, "\0", 2);
          }
#else
          write(outfd, "0", 1);
