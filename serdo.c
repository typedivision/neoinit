...
int batch(char *s) {
  ...
  char *map;
  if (fd == -1) {
    diesys(1, "could not open ", s);
  }
  if (fstat(fd, &ss) == -1) {
    diesys(1, "could not stat ", s);
  }
  if (ss.st_size > 32768) {
    die(1, "file ", s, " is too large");
  }
  map = alloca(ss.st_size + 1);
  if (read(fd, map, ss.st_size) != (long)ss.st_size) {
    diesys(1, "read error");
  }
  map[ss.st_size] = 0;
  close(fd);
